...
public class Menu extends JComponent {
				...
				if (com instanceof MenuItem) {
					...
					if (model != null) {
						for (int i = 0; i < model.getSize(); i++) {
							ModelSubMenu data = new ModelSubMenu(getMenuIcon(icon, i),
	...
	protected void paintComponent(Graphics g) {
		...
		if (getComponentCount() > 1) {
			...
			g2.setPaint(new GradientPaint(x, y, animator.getSelectedColor1(), x + 3, y + height,
					animator.getSelectedColor2()));
	...
	private Shape createStyle() {
		...
		GeneralPath p = new GeneralPath(
				new CubicCurve2D.Double(width, y, width, y + 50, x, y + space, x, y + 50 + space));
		p.lineTo(x, lastItemLocation);
		p.append(new CubicCurve2D.Double(x, lastItemLocation, x, lastItemLocation + 50, width, lastItemLocation + space,
				width, lastItemLocation + 50 + space), true);
		return p;
	}

	private Icon getMenuIcon(ListModel<String> data, int index) {
		if (data != null && index < data.getSize()) {
			return new BufferedImageIO().getIcon(20, 20, data.getElementAt(index));
		} else {
			return null;
		}
	}
