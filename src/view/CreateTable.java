...
public class CreateTable implements MouseListener, KeyListener {
	...
	public JPanel east() {
		...
		limitLabe = new JLabel("Limit");
		limitTextField = new JTextField("2", 5);
		limitTextField.addKeyListener(this);
		JLabel dtype = new JLabel("Data type");
		JLabel addC = new JLabel("Add constraints");
		String datatype[] = { "int", "varchar", "real", "blob", "decimal", "date" };
		_data = new JComboBox<String>(datatype);
		_data.addActionListener((ActionEvent e) -> {
			ctm.setDatatype(_data.getSelectedItem().toString());
			if (hasLimit(ctm.getDatatype())) {
		...
		eastPanel.setLayout(new FlowLayout(0));
